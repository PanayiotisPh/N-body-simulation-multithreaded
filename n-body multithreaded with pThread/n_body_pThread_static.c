...
void computeAccelerations(int id)
{
    int i, j;
    int start= ceil(1.0*bodies/NUM_THREADS)*id ;
    int end= ceil(1.0*bodies/NUM_THREADS)*(id+1);
    if(end > bodies){
	end=bodies;
    }
    for (i = start; i < end; i++)
...
int main(int argc, char *argv[])
{
    ...
    printBodiesInfo(lfp, dfp);
        ...
        {
	thread_id[i]=i;
	pthread_create(&threads[i], NULL, (void *)simulate, &thread_id[i]);
	}
  	for (i = 0; i < NUM_THREADS; i++)
    	{
