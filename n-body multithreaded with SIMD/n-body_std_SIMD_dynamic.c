...
void resolveCollisions(int noOfThreads)
{
    ...
    #pragma omp for schedule(dynamic) reduction(+: changes)
        for (i = 0; i < bodies - 1; i++)
        {
            changes = 0;
...
{
    int i, j;
    #pragma omp parallel for schedule(dynamic, bodies / noOfThreads) num_threads(noOfThreads) shared(accelerations, positions_x, positions_y, positions_z, masses) private(i, j)
    for (i = 0; i < bodies; i++)
    {
