...
void computeAccelerations(int no_of_threads){
	int i,j;
    omp_set_num_threads(no_of_threads);
    #pragma omp parallel for schedule(static,bodies/no_of_threads) private(j) shared(positions, accelerations, masses)
	for(i=0;i<bodies;i++){
...
void simulate(int no_of_threads){
	SimulationTime++;
	computeAccelerations(no_of_threads);
	computePositions();
	computeVelocities();
	resolveCollisions();
